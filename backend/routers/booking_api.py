...
async def fetch_bookings_of_current_user(current_user_data: validation_models.TokenData = Depends(oauth2.get_current_user)):
...
async def create_booking_by_current_user(booking: validation_models.Booking, current_user_data: validation_models.TokenData = Depends(oauth2.get_current_user)):
...
async def update_booking_by_id(booking_id: int, booking: validation_models.Booking, current_user_data: validation_models.TokenData = Depends(oauth2.get_current_user)):
...
async def delete_booking_by_id(booking_id: int, current_user_data: validation_models.TokenData = Depends(oauth2.get_current_user)):
