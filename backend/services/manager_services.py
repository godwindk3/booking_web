...
def get_manager_by_id(manager_id: int):
    temp = db.query(data_models.Manager).filter(data_models.Manager.id == manager_id).first()

    if (temp is None):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Manager with ID {manager_id} does not exist.")
...
def create_manager(manager: validation_models.Manager):
    ...
    if (temp is None):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User with ID {manager.userID} does not exist.")
    
    temp = db.query(data_models.Accommodation).filter(data_models.Accommodation.id == manager.accommodationID).first()
    if (temp is None):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Accommodation with ID {manager.accommodationID}")
